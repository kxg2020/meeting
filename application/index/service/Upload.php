...
class Upload{
    ...
    private $fileExt;
    private $extension= 'bmp,gif,jpg,jpeg,pic,png,tif,wmv,asf,asx,rm, rmvb,mp4,mp3,3gp,mov,m4v,avi,dat,mkv,flv,vod,txt,doc,xls,ppt,docx,xlsx,pptx,pdf';

    public function uploadFile(){
        if($this->checkPath()){
            $file = Request::file("file");
            $root = Url::build("index/index");
            $info = $file->validate(['size'=>$this->fileSize,'ext'=>$this->extension])->move($this->savePath,"");
            if($info){
